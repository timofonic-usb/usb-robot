...
typedef struct {
        ...
        static
char *data_read_binary(command_context * context, size_t size)
{
        ...
        static
char *data_read_hex(command_context * context, size_t size)
{
        int c, i = 0;
        char *buffer = malloc(size);
...
static const reader_wrapper readers[] = {
        ...
        static
void data_write_binary(command_context * context, size_t size, char *buffer)
{
        ...
        static
void data_write_hex(command_context * context, size_t size, char *buffer)
{
        int i;
...
static const writer_wrapper writers[] = {
        ...
        static
char *next_word_start(char *buffer)
{
        char *pointer = buffer;
...
//XXX ph make inline?
        static
long read_integer(char const *buffer)
{
        return strtol(buffer, NULL, 0);
}

        static
char const *read_param(char const *name, char const *buffer)
{
        ...
        static
int command_change_output(command_context * context, char *buffer)
{
        ...
        static
int command_change_input(command_context * context, char *buffer)
{
        ...
        static
int command_set_config(command_context * context, char *buffer)
{
        int confignumber;
        ...
        static
int command_claim_interface(command_context * context, char *buffer)
{
        int interface;
        ...
        static
int command_abort(command_context * context, char *buffer)
{
        ...
        static
int command_quit(command_context * context, char *buffer)
{
...
typedef struct {
        ...
        static
int param_int(char const *buffer, void *value)
{
        ...
        *integer = read_integer(buffer);
        ...
        static
int param_char(char const *buffer, void *value)
{
        ...
        *character = *buffer;
        ...
        static
int param_direction(char const *buffer, void *value)
{
        ...
        switch (tolower(*buffer)) {
                ...
                case 'i':
                        *dir = dir_in;
                        break;
                default:
                        error("bad initial direction letter, %c", *buffer);
                        return -1;
        }
        ...
        static
int param_transfer_type(char const *buffer, void *value)
{
        ...
        static
int command_transfer(command_context * context, char *buffer)
{
        long timeout = 10000;
        ...
        switch (dir) {
                case dir_in:
                        ...
                        break;
                case dir_out:
                        data = context->read(context, size);
                        break;
                default:
                        cant_get_here();
                        break;
        ...
        switch (ttype) {
                case transfer_ctrl:
                        message("doing control message id %d %s device, size %d, timeout %d frames, %x:%x:%x:%x",
                                        (int)context->id,
                                        direction_to_string[dir], (int)size, (int)timeout, requesttype, request, value, index);
                ...
                case transfer_bulk:
                        message("doing bulk transfer id %d %s ep 0x%x, size %d, timeout %d frames",
                                        (int)context->id, direction_to_string[dir], (int)ep, (int)size, (int)timeout);
                        ...
                        switch (dir) {
                                case dir_in:
                                        if ((read = usb_bulk_read(context->device, ep, data, size, timeout)) != size) {
                                                if (read > 0)
                                                        usb_error("problem doing read; only %d read from %d", (int)read,
                                                                        (int)size);
                                                else
...
static
int command_help(command_context * context, char *buffer);
...
static const command commands[] = {
        ...
        {"decoding", "change decoding of data input from console\n" "\tPossible parameters: binary,hex", command_change_input},
        {"encoding", "change encoding of data output to console\n" "\tPossible parameters: binary,hex", command_change_output},
...
int command_help(command_context * context, char *buffer)
{
        ...
        static
void ub_getline(char **buffer, size_t * buffer_length, command_context * context)
{
        ssize_t length = 0;
...
got_line:continue;
        ...
        static
int do_command(command_context * context)
{
        char *buffer = NULL;
        ...
        for (i = commands; i->name; i++) {
                ...
                switch (i->action(context, next_word_start(buffer))) {
                        case 0:
                                fprintf(context->out, "OK: id=%d\n", (int)context->id);
                                goto cleanup;
                        case -2:	// syntax error in command
                        case -1:
                                fprintf(context->out, "ERROR: id=%d\n", (int)context->id);
                                goto cleanup;
                        case 2:	// abort
                                return_value = 2;
                                goto cleanup;
...
int control_device(struct usb_dev_handle *handle)
{
        int status;
