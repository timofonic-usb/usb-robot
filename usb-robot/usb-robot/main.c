...
const char *value_type_string_table[] = {
        ...
        static
void option_match(const option * opt, char *argument)
{
        char *pointer;
        long temp;
        ...
        static
void action_idvendor(void *integer)
{
        ...
        static
void action_idproduct(void *integer)
{
        int *value = (int *)integer;
        idProduct = *value;
}
        ...
        static
void action_print_version(void *unused)
{
...
void print_help_and_exit();
        ...
        static
void action_print_help_and_exit(void *unused)
{
        print_help_and_exit();
}
...
static void print_help_and_exit(void)
{
        ...
        for (opt = command_line_opts; opt->string; opt++) {
                message("\t%s%s: %s", opt->string, value_type_string_table[opt->value_type], opt->explanation);
        }
        exit(EXIT_SUCCESS);
}

static usb_dev_handle *open_device(struct usb_device *device)
{
        usb_dev_handle *handle;

        message("opening device %s on bus %s", device->filename, device->bus->dirname);
...
static int scan_bus(struct usb_bus *bus)
{
        ...
        for (device = roottree; device; device = device->next) {
                if (((idVendor == -1) ? 1 : (device->descriptor.idVendor == idVendor)) &&
                        ...
                        message("found device %s on bus %s (idVendor 0x%x idProduct 0x%x)",
                                        device->filename,
                                        device->bus->dirname, device->descriptor.idVendor, device->descriptor.idProduct);
                        ...
                        message("device %s on bus %s does not match", device->filename, device->bus->dirname);
        }
